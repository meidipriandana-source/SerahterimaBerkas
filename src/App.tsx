...
export default function App() {
  ...
  return (
    <div className={`min-h-screen font-sans text-slate-900 transition-all duration-300 ${
      isSmartphoneMode 
      ...
      <div className={`
        flex flex-col overflow-hidden bg-slate-50 relative
        ${isSmartphoneMode 
      ...
      `}>
        ...
        {isSmartphoneMode && (
          <>
            ...
            <div className="hidden lg:flex h-9 bg-white text-slate-800 px-6 pt-2 items-center justify-between text-[10px] font-bold select-none shrink-0 z-40 border-b border-slate-100">
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <header className="bg-white border-b border-slate-200/80 text-slate-800 shadow-3xs sticky top-0 z-40 shrink-0">
            <div className="px-5 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <button 
                  ...
                  className={`p-1.5 hover:bg-slate-100 rounded-lg transition cursor-pointer text-slate-600 ${(!isSmartphoneMode) ? "lg:hidden" : ""}`}
                >
                  {isMobileMenuOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
                </button>
                
                <div className="w-14 h-14 rounded-xl overflow-hidden flex items-center justify-center shadow-xs bg-slate-50 border border-slate-100 p-1 shrink-0">
                ...
                <div className="flex flex-col">
                  <div className="text-sm font-black tracking-wider text-indigo-600 font-mono flex items-center gap-1.5 leading-none">
                    <Clock className="w-3.5 h-3.5 text-indigo-500 animate-pulse shrink-0" />
                    <span>
                      {time.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}
                    </span>
                  </div>
                  <span className="text-[9px] text-slate-500 font-bold tracking-tight mt-1">
          ...
          <div className={`flex-1 flex flex-col relative overflow-hidden ${(!isSmartphoneMode) ? "lg:flex-row" : ""}`}>
            ...
            <aside className={`
              shrink-0 bg-white border-r border-slate-200/80 text-slate-600 flex flex-col z-30
            ...
            `} id="sidebar-navigation">
              ...
              <div className="p-4.5 border-b border-slate-100 flex items-center gap-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white shrink-0">
                <div className="w-14 h-14 bg-white rounded-xl flex items-center justify-center p-1 shadow-3xs border border-white/20 overflow-hidden shrink-0">
              ...
              <nav className="flex-1 p-3.5 space-y-1 overflow-y-auto">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block px-3 mb-2 font-display">
                  Menu Utama
                </span>
                {tabs.map((tab) => {
                  const Icon = tab.icon;
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 cursor-pointer ${
                        isActive 
                          ? "bg-indigo-50/80 text-indigo-700 shadow-3xs border-l-4 border-indigo-600 font-extrabold" 
                    ...
                    >
                      ...
                      {tab.count !== null && tab.count > 0 && (
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold ${isActive ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-800'}`}>
                          {tab.count}
                        </span>
                      )}
                    </button>
              ...
              <div className="p-3 border-t border-slate-100 bg-slate-50 text-[11px] text-slate-500 space-y-1.5 shrink-0">
                <div className="flex items-center justify-between font-bold text-slate-600">
                  <span>SINKRONISASI AKTIF</span>
                  <span className="text-[9px] bg-indigo-100 text-indigo-700 px-1.5 rounded">AUTO</span>
                </div>
            ...
            {/* Content Panel Box */}
            <main className="flex-1 p-4 overflow-y-auto max-w-full bg-slate-50">
                ...
                <div className="flex flex-col justify-between items-center bg-white p-3 rounded-xl border border-slate-200 shadow-xs gap-3">
                  <div className="flex flex-wrap gap-2 justify-center">
                     <div className="flex items-center gap-1.5">
                       ...
                       <span className="text-[10px] text-slate-500">G-Sheet DB Connected</span>
                     </div>
                     <div className="flex items-center gap-1.5">
                       <div className="w-1.5 h-1.5 rounded-full bg-indigo-500"></div>
                       <span className="text-[10px] text-slate-500">AppSheet Active</span>
                     </div>
                  </div>
        ...
        {isSmartphoneMode && (
          <div className="hidden lg:flex h-5 bg-white flex items-center justify-center shrink-0 z-40 border-t border-slate-50">
